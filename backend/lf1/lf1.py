...
ES_HOST = 'https://vpc-photos-lcmy4qhknluzuuouq3pufgnnk4.us-east-1.es.amazonaws.com'
REGION = 'us-east-1'
...
def lambda_handler(event, context):
    ...
    bt3 = boto3.session.Session('','',region_name='us-east-1')
    rek = bt3.client('rekognition')
    ...
    for record in event['Records']:
        ...
        print(bucket,key,size)
        # detect the labels of current image
        labels = rek.detect_labels(Image={'S3Object': { 'Bucket': bucket,'Name': key}},MaxLabels=10)
        print(labels)
    ...
    for label in labels['Labels']:
        ...
        obj["labels"].append(label['Name']+'s')
